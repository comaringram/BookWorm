...
Overall: This will be implementd using LWCs on a Lighting App Page, and customer facing through Experience.
 ...
 Search: For search, we will implement a design that allows users to search using Title, Author, ISBM number. These fields will also need to be fields within the Salesforce org under the Book Object. Imperative Apex herre
 View Inventory: For this requirement, we will implement imperastive Apex again, with pagination. We want to get 10 or 15 records6 with each click, so that the users ins't waiting for resuklts as we scale inveteory. 
